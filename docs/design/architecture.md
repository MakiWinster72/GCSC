...
本页面提供了 BDAI-SC 学生活动中心内部结构的高层架构图——即将其 Spring Boot 后端和 Vue 3 前端连接为一个单一、连贯系统的分层模式。

## 技术栈概览

BDAI-SC 是一个单体全栈应用，分为两个独立进程：一个 Spring Boot REST API 和一个 Vue 3 单页应用。
...
BDAI-SC 中最独特的架构决策是其**多实体成果系统**。它没有使用单一的多态 achievements 表，而是让每个成果类别映射到自己的 JPA 实体、数据库表和 Repository。
